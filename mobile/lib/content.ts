import { commonBigrams, type Difficulty, type TestCategory } from "../constants/options";

const sampleText: Record<TestCategory, Record<Difficulty, string[]>> = {
  "Fjale te zakonshme": {
    Lehte: [
...
export function generateTestText(category: TestCategory, difficulty: Difficulty, wordsTarget = 92) {
  const source = sampleText[category][difficulty].join(" ").split(/\s+/);
  const multiplier = difficulty === "Lehte" ? 0.85 : difficulty === "Mesatar" ? 1.08 : 1.32;
...
export function generateBigramText(rounds = 10) {
  const parts: string[] = [];
...
export function generateWeakKeyText(keys: string[]) {
  const usable = keys.length > 0 ? keys : ["a", "s", "d", "f", "j", "k", "l"];
  const parts: string[] = [];
  for (let round = 0; round < 12; round++) {
    for (const key of usable.slice(0, 8)) {
      const label = key === "space" ? " " : key;
      parts.push(`${label}a`, `a${label}`, `${label}e`, `e${label}`);
    }
  }
